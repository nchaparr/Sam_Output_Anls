...
Ax.set_xlabel(r"$\frac{\partial \overline{\theta}}{\partial z}/\gamma$ Km$^{-1}$", fontsize=25)
#Ax.set_xlabel(r"$\overline{w^{,}\theta^{,}}$", fontsize=20)
#Ax1.set_ylabel(r"$\frac{z}{h}$", fontsize=20)
Ax.set_ylabel(r"$\frac{z}{h}$", fontsize=30)
#plt.xlim(0, 1)
plt.xlim(-.08, 2)
#plt.ylim(50, 950)
plt.ylim(0.2, 1.4)
...
for i in range(len(theta_file_list)):
   ...
   #Now for the gradients
    ...
    element0 = np.array([0])
    dthetadz=np.hstack((element0, 1.0*dthetadz/gamma)) #*1.0
