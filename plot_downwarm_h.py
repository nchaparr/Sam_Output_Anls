...
def Main_Fun(rundate, gamma, flux_s):
     ...
     if rundate=="Jan152014_1":
         scaled_time, upwarm_h0 = scaled_time[0:11], upwarm_h0[0:11] 
     
     return upwarm_h0, scaled_time    
...
for run in run_list:
    #print run[0]
    upwarm_h0, Times = Main_Fun(run[0], run[1], run[2])
    Ax3.plot(Times, 1.0*upwarm_h0, run[4], markersize=10)
