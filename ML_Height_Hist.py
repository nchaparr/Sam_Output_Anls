...
theAx.set_title(r"$\gamma = 10 \ (Kkm^{-1})$", fontsize=30)
#theAx.set_ylabel(r'$P( \frac{h^{l}_{0}}{h} )$', fontsize=30)
theAx.set_ylabel(r'$Count$', fontsize=30)
theAx.set_xlabel(r'$h^{l}_{0}$', fontsize=30)
theAx.set_xlim(400, 1400) #TODO:need to test axis limts first
theAx.set_ylim(0, 25000)

#theAx.set_xlim(0.4, 1.2) #TODO:need to test axis limts first
#theAx.set_ylim(0, .1)
...
theAx.tick_params(axis='both', which='major', labelsize=20)
    ...
    for i in range(len(dump_time_list)):
        ...
        if i == 4:
          ...
          #theAx.set_title('Histogram of local Mixed Layer Heights from 1 Case at 5 hrs')
            theAx.bar(1.0*height, 1.0*height_bin_vols, width = Width, alpha = Alpha, color=Color, label=Label)     
theAx.legend(loc = 'upper right', prop={'size':20})
theFig.tight_layout()          
theFig.show()          
