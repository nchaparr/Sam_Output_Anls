...
parser.add_argument('--case',type=int,help='casenumber')
parser.add_argument("--lev",type=str, help="zf0 or zg0")
...
with h5py.File(h5_profiles,'r') as infile:
    ...
    case_list=[case for count,case  in enumerate(case_tups)]
    print(case_list)
    case=case_list[case][1]
...
# zenc=df_0['zenc_nd_height'].values
# target=var_dict[case]['height_dict'][height]/var_dict[case]['height_dict']['zenc']
# zlevs=df_0['zenc_nd_height'].values
# index=int(np.searchsorted(zlevs,target))

# wpert=var_dict[case][index]['wptp']['wpert']
# thetapert=var_dict[case][index]['wptp']['thetapert']
# plt.close('all')
# fig,(ax1,ax2)=plt.subplots(2,1)
# ax1.hist(wpert,bins=30)
# ax2.hist(thetapert,bins=30)
# title='{}  {}  {}'.format(case,height,nd_time)
# fig.suptitle(title)
# plt.show()

# rms = float(np.sqrt(np.mean(wpert**2.)))
# std = float(np.std(wpert))
# print(title,rms,std)
